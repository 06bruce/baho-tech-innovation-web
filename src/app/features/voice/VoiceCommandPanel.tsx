...
export function VoiceCommandPanel() {
  ...
  const handleUnhandledCommand = useCallback(
    ...
    [changeLanguage, handleLogout, navigate, user]
  );
  const commands = useMemo(
    ...
    [handleLogout, navigate, user?.disabilityCategory, user?.role]
  );
  ...
  return (
    <section className="rounded-3xl border border-[#d8e4ec] bg-white p-5 shadow-sm" aria-labelledby="voice-command-title">
      <h3 id="voice-command-title" className="text-xl font-semibold text-gray-950">{t("voice.title")}</h3>
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        ...
        <button type="button" onClick={stop} disabled={!supported || !isListening} className="inline-flex items-center gap-2 rounded-full border border-[#1A4F8D] px-5 py-2.5 font-semibold text-[#1A4F8D] hover:bg-[#eef5f9] focus:outline-none focus:ring-4 focus:ring-[#1A4F8D]/25 disabled:opacity-60">
          <Square className="h-4 w-4" aria-hidden="true" /> {t("voice.stop")}
        </button>
      </div>
      <details className="mt-4">
        <summary className="cursor-pointer font-semibold text-[#1A4F8D]">{t("voice.available")}</summary>
        <ul className="mt-3 grid gap-2 text-sm text-gray-700 sm:grid-cols-2">
          {commands.map((command) => <li key={command.phrase} className="rounded-2xl bg-[#F5F7FA] px-3 py-2">{command.phrase}</li>)}
        </ul>
      </details>
    </section>
  );
