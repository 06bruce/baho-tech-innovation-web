  ...
  return (
    <section className="rounded-3xl border border-[#d8e4ec] bg-white p-6 shadow-sm dark:border-white/10 dark:bg-[#0B1F33]" aria-labelledby="stt-title">
      ...
      <div className="mt-4 space-y-3">
        {!supported && fallbackSupported && <FormAlert tone="info">{t("speech.firefoxFallback")}</FormAlert>}
        {!supported && !fallbackSupported && <FormAlert tone="error">{t("speech.unsupportedStt")}</FormAlert>}
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        <button type="button" onClick={() => (supported ? start(language) : void startFallbackRecording())} disabled={(!supported && !fallbackSupported) || isListening} className="inline-flex items-center gap-2 rounded-full bg-[#1A4F8D] px-6 py-3 font-semibold text-white hover:bg-[#1C5B78] focus:outline-none focus:ring-4 focus:ring-[#1A4F8D]/25 disabled:opacity-60">
          <Mic className="h-5 w-5" aria-hidden="true" /> {t("speech.startListening")}
        </button>
        <button type="button" onClick={() => (supported ? stop() : token ? void stopFallbackRecording({ token, language }) : undefined)} disabled={(!supported && !fallbackSupported) || !isListening} className="inline-flex items-center gap-2 rounded-full border border-[#1A4F8D] px-6 py-3 font-semibold text-[#1A4F8D] hover:bg-[#eef5f9] focus:outline-none focus:ring-4 focus:ring-[#1A4F8D]/25 disabled:opacity-60 dark:border-[#FEC629] dark:text-[#FEC629] dark:hover:bg-white/10">
      ...
      <div className="mt-6 min-h-64 rounded-2xl bg-[#F5F7FA] p-5 text-lg leading-8 text-gray-950 dark:bg-[#102A43] dark:text-white" aria-live="polite">
        ...
        {visibleTranscript || t("speech.transcriptEmpty")}
      </div>
